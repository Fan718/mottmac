...
### 2.2.1 Pipe Structual Peak Grade
```{r}
...
# According to the Manual, the mean score is 
# Check missing values
...
### 2.2.2 Pipe Serviceability Peak Grade
```{r}
...
### 2.3.2 Pipe Material
```{r}
# We have two columns related to pipe materials in the data set,
# one is from our system and the other is from AC GIS.
# And we decided to use the latter because it's more general which can be applied to any other pipe infrastructures. 
...
ggplot(df5, aes(x=inspection_date, y=pipe_length)) + 
  geom_point()
...
```{r}
# Column SW_SHAPE
# Based on Table2.11 from 3rd edition of NZ Pipe Inspection Manual,
# there are 7 categories of pipe shape, CIRC (Circular Pipe), EGG(Egg-Shaped),OVAL(Oval-Shaped), RECT(Rectangular Box), BOX(Square Box), USHP(U-Shaped), and others.
...
### 2.3.8 Categorical Varibles
```{r}
# Column SW_CRE - Consolidated receiving environment
df7 %>% count(SW_CRE)
# plot
ggplot(df7, aes(x=SW_CRE, fill=structural_grade)) +
  geom_bar() +
  scale_fill_brewer(palette = "Dark2") +
...
df7 %>% 
  ...
  summarise(n=n())
# Bar Plot 
ggplot(df7, aes(x=SW_OPERATIONAL_AREA, fill=SW_OPERATIONAL_AREA)) +
  geom_bar() +
  scale_fill_brewer(palette = "Dark2")
```
...
clean_df <- subset(df7, select = c(install_date, years,
                                   material, diameter,
...
write.csv(clean_df, 
          "C:/Users/Claire/Desktop/Mott MacDonald\\SW_Pipe.csv",
          row.names = FALSE)
